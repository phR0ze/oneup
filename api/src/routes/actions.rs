...
{
  let id = model::action::insert(state.db(), &action.name).await?;
...
{
  Ok(Json(model::action::update_by_id(state.db(), action.id, &action.name).await?))
}
...
mod tests {
  ...
  async fn test_delete_by_id() {
    ...
    let action1 = "action1";
    let id = model::action::insert(state.db(), action1).await.unwrap();
  ...
  async fn test_update_by_id() {
    ...
    let req = Request::builder().method(Method::PUT)
      ...
      .body(Body::from(serde_json::to_vec(&serde_json::json!(
          model::UpdateAction { id: id, name: format!("{action2}") })
      ).unwrap())).unwrap();
    let res = routes::init(state.clone()).oneshot(req).await.unwrap();
    assert_eq!(res.status(), StatusCode::OK);
  ...
  async fn test_get_all_success() {
    ...
    let action1 = "action1";
    let action2 = "action2";
    model::action::insert(state.db(), action2).await.unwrap();
    model::action::insert(state.db(), action1).await.unwrap();
  ...
  async fn test_create_failure_no_name_given() {
    ...
    // Attempt to create a Action with no name
    let req = Request::builder().method(Method::POST)
      .uri("/actions").header("content-type", "application/json")
      .body(Body::from(serde_json::to_vec(&serde_json::json!(
        model::CreateAction { name: "".to_string() }
      )).unwrap())).unwrap();
    ...
    let bytes = res.into_body().collect().await.unwrap().to_bytes();
    let simple: model::Simple = serde_json::from_slice(&bytes).unwrap();
    assert_eq!(simple.message, "Action name value is required");
  }
  ...
  async fn create_action_req(state: Arc::<state::State>, name: &str) -> Response<Body> {
    let req = Request::builder().method(Method::POST)
      .uri("/actions").header("content-type", "application/json")
      .body(Body::from(serde_json::to_vec(&serde_json::json!(
        model::CreateAction { name: format!("{name}") }
      )).unwrap())).unwrap();
